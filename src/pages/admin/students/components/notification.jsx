...
const Notification = () => {
          ...
          <div key={status} className="mb-6 bg-white rounded shadow">
            ...
            {isOpen && (
              <div className="p-4 space-y-4">
                {requests.length === 0 ? (
                  <p className="text-gray-500">No {status.toLowerCase()} requests available.</p>
                ) : (
                  requests.map((req) => {
  ...
  return (
    ...
    >
      ...
      <p><strong>Roll No.:</strong> {req.registerNumber}</p>
...
>
  {expandedId === req._id ? "View Less" : "View More"}
</button>
