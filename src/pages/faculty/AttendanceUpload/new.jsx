...
// =================================================================================================
// HISTORY REPORT SCREEN (With Hour Grouping)
// =================================================================================================
const HistoryReportScreen = ({ history, onBack }) => {
  ...
  const filteredHistory = useMemo(() => {
    if (!showData) return [];
    return history.filter(rec => rec.date === toYYYYMMDD(filters.date) && rec.batch === filters.batch && rec.courseCode === filters.courseCode);
  ...
  const groupedRecords = useMemo(() => {
    ...
    filteredHistory.forEach(rec => {
      if (studentMap.has(rec.studentId)) {
        studentMap.get(rec.studentId).hours.push(rec.hour);
      } else {
        studentMap.set(rec.studentId, { ...rec, hours: [rec.hour] });
      }
    });
  ...
  const handleFilterChange = (field, value) => {
    setCurrentPage(1);
    setFilters(prev => { const newFilters = { ...prev, [field]: value }; if (field === 'date') { newFilters.batch = 'All'; newFilters.courseCode = 'All'; } if (field === 'batch') { newFilters.courseCode = 'All'; } return newFilters; });
  };
  
  const downloadCSV = () => {
    if (groupedRecords.length === 0) return;
    const headers = ['Date', 'Batch', 'Course Code', 'Course Name', 'Hours', 'Roll No', 'Student Name', 'Status'];
    const csvContent = [ headers.join(','), ...groupedRecords.map(row => [ row.date, row.batch, row.courseCode, `"${row.courseName}"`, row.hours.sort((a,b)=>a-b).join(' & '), row.studentId, row.studentName, row.status ].join(',')) ].join('\n');
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `attendance_history_${toYYYYMMDD(new Date())}.csv`);
    ...
    <div className="p-8 mt-20">
      ...
      <div className="overflow-x-auto border border-gray-200 rounded-lg">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50"><tr>{['Date', 'Batch', 'Course', 'Hour(s)', 'Roll No', 'Student Name', 'Status'].map(header => <th key={header} className="px-6 py-3 text-xs font-bold tracking-wider text-left text-gray-600 uppercase">{header}</th>)}</tr></thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {showData && currentRecords.length > 0 ? currentRecords.map((rec) => (
              <tr key={rec.studentId} className="hover:bg-gray-50">
                <td className="px-6 py-4 text-sm text-gray-700 whitespace-nowrap">{rec.date}</td>
                <td className="px-6 py-4 text-sm text-gray-700 whitespace-nowrap">{rec.batch}</td>
                <td className="px-6 py-4 text-sm font-medium text-gray-900 whitespace-nowrap">{rec.courseCode}</td>
                <td className="px-6 py-4 text-sm text-center text-gray-700 whitespace-nowrap">{rec.hours.sort((a, b) => a - b).join(' & ')}</td>
                <td className="px-6 py-4 text-sm font-medium text-gray-800 whitespace-nowrap">{rec.studentId}</td>
                <td className="px-6 py-4 text-sm text-gray-800 whitespace-nowrap">{rec.studentName}</td>
                <td className="px-6 py-4 text-sm font-semibold whitespace-nowrap"><span className={`px-2 py-1 text-xs rounded-full ${rec.status === 'P' ? 'bg-green-100 text-green-800' : rec.status === 'A' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}`}>{rec.status}</span></td>
              </tr>
            )) : (<tr><td colSpan="7" className="p-8 text-center text-gray-500">{!showData ? "Please select a date, batch, and course to view records." : "No records match your filters."}</td></tr>)}
          </tbody>
        </table>
      </div>
      {showData && groupedRecords.length > recordsPerPage && (<div className="flex items-center justify-between px-4 py-3 mt-4 bg-white border-t border-gray-200 rounded-lg"><button onClick={() => setCurrentPage(prev => prev - 1)} disabled={currentPage === 1} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50">Previous</button><div className="text-sm text-gray-700">Page <span className="font-medium">{currentPage}</span> of <span className="font-medium">{totalPages}</span></div><button onClick={() => setCurrentPage(prev => prev + 1)} disabled={currentPage === totalPages} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50">Next</button></div>)}
    </div>
  );
};
...
const SelectHourScreen = ({ onProceed, onBack, hourBlocks }) => {
  ...
  return ( <div className="p-8"> <div className="pb-4 mb-6 border-b border-gray-200"><h2 className="text-2xl font-bold text-gray-800">Select Hour Block</h2>{courseInfo && <p className="text-gray-500">Choose the class hours for {courseInfo.courseCode} - {courseInfo.courseName}.</p>}</div> <div className="space-y-4">{hourBlocks.length > 0 ? hourBlocks.map((block, index) => (<label key={index} className="flex items-center p-5 border border-gray-300 rounded-lg cursor-pointer has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50"><input type="checkbox" checked={selectedBlock === block} onChange={() => handleCheckboxChange(block)} className="w-5 h-5 mr-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500"/><h3 className="text-lg font-semibold text-gray-900">{formatBlockText(block)}</h3></label>)) : <p className="p-4 text-center text-gray-500 bg-gray-100 rounded-md">No available hours for this course, or attendance has already been submitted.</p>}</div> <div className="flex justify-between mt-8"><button onClick={onBack} className="px-6 py-2 font-semibold text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300">← Back</button><button onClick={() => onProceed(selectedHourIds)} disabled={selectedHourIds.length === 0} className="px-6 py-2 font-semibold text-white bg-blue-600 rounded-md shadow-sm hover:bg-blue-700 disabled:bg-blue-300">Proceed →</button></div></div> );
};
...
const MarkAttendanceScreen = ({ selectedClasses, students, onBack, onSubmit }) => {
  ...
  return ( <div className="p-8 mt-20"> <div className="pb-4 mb-6 border-b border-gray-200">{courseInfo && <h2 className="text-2xl font-bold text-gray-800">Mark Attendance: {courseInfo.courseCode}</h2>}<p className="text-gray-500"><strong>Hour(s):</strong> {selectedHoursText}</p></div> <div className="overflow-hidden border border-gray-200 rounded-lg"><table className="min-w-full divide-y divide-gray-200"><thead className="bg-gray-50"><tr>{['S.No', 'Roll No', 'Student Name', 'Status'].map(header => <th key={header} className="px-6 py-3 text-xs font-bold tracking-wider text-left text-gray-600 uppercase">{header}</th>)}</tr></thead><tbody className="bg-white divide-y divide-gray-200">{students.map((student, index) => (<tr key={student.id} className="hover:bg-gray-50"><td className="px-6 py-4 text-sm text-gray-500">{index + 1}</td><td className="px-6 py-4 text-sm font-medium text-gray-900">{student.id}</td><td className="px-6 py-4 text-sm text-gray-800">{student.name}</td><td className="px-6 py-4"><div className="flex items-center space-x-4">{['P', 'A', 'OD'].map(status => (<label key={status} className="flex items-center space-x-2 text-sm cursor-pointer"><input type="radio" name={`status_${student.id}`} checked={attendance[student.id] === status} onChange={() => handleStatusChange(student.id, status)} className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"/><span>{status}</span></label>))}</div></td></tr>))}</tbody></table></div> <div className="flex justify-between mt-8"><button onClick={onBack} className="px-6 py-2 font-semibold text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300">← Back</button><button onClick={() => onSubmit(attendance)} className="px-6 py-2 font-semibold text-white bg-blue-600 rounded-md shadow-sm hover:bg-blue-700">Submit</button></div></div> );
};
...
const AttendanceUploadPage = ({ onNewSubmission, submittedHours }) => {
  ...
  const classesForSelectedBatch = useMemo(() => { if (!filters.batch) return []; return classesOnSelectedDay.filter(c => c.batch === filters.batch); }, [classesOnSelectedDay, filters.batch]);
  const availableCourses = useMemo(() => getUniqueObjects(classesForSelectedBatch, 'courseCode'), [classesForSelectedBatch]);
  const studentsToDisplay = useMemo(() => { if (!filters.batch || !filters.courseCode) return []; return mockStudentList.filter(student => student.batch === filters.batch && student.enrolledCourses.includes(filters.courseCode)); }, [filters.batch, filters.courseCode]);
  const hourBlocks = useMemo(() => { const availableHours = masterTimetable .filter(c => c.courseCode === filters.courseCode && c.day === dayOfWeek && c.batch === filters.batch && !submittedHours.includes(c.id)) .sort((a, b) => a.hour - b.hour); if (availableHours.length === 0) return []; return availableHours.reduce((acc, currentHour) => { const lastBlock = acc[acc.length - 1]; if (lastBlock && currentHour.hour === lastBlock[lastBlock.length - 1].hour + 1) { lastBlock.push(currentHour); } else { acc.push([currentHour]); } return acc; }, []); }, [filters.courseCode, dayOfWeek, filters.batch, submittedHours]);
  ...
  const handleHourSelect = (selectedHourIds) => { setSelectedClasses(masterTimetable.filter(c => selectedHourIds.includes(c.id))); setStep(3); };
  const handleSubmit = (attendanceData) => { onNewSubmission({ filters, selectedClasses, students: studentsToDisplay, attendanceData }); alert('Attendance submitted successfully!'); setStep(1); setSelectedClasses([]); };
  const goBack = () => { if (step === 3) setSelectedClasses([]); setStep(prev => prev - 1); };
  return ( <> {step === 1 && <SelectCourseScreen filters={filters} handleDateChange={handleDateChange} handleBatchChange={handleBatchChange} handleCourseChange={handleCourseChange} onProceed={handleProceedToHourSelection} availableBatches={availableBatches} availableCourses={availableCourses} />} {step === 2 && <SelectHourScreen onProceed={handleHourSelect} onBack={goBack} hourBlocks={hourBlocks} />} {step === 3 && <MarkAttendanceScreen selectedClasses={selectedClasses} students={studentsToDisplay} onBack={goBack} onSubmit={handleSubmit} />} </> );
};
...
const AttendanceManager = () => {
  ...
  return ( <div className="min-h-screen bg-gray-100 mt-20"> <main className="max-w-6xl p-4 mx-auto"> <div className="bg-white rounded-lg shadow-md"> {view === 'upload' && (<><div className="flex justify-end p-4 border-b border-gray-200"><button onClick={() => setView('history')} className="font-semibold text-blue-600 hover:text-blue-800">View History Report →</button></div><AttendanceUploadPage onNewSubmission={handleNewSubmission} submittedHours={submittedHours} /></>)} {view === 'history' && <HistoryReportScreen history={attendanceHistory} onBack={() => setView('upload')} />} </div> </main> </div> );
};
