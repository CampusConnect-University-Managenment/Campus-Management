...
const AttendanceManager = () => {
  ...
  // Function to handle individual student attendance marking (temporary state before upload)
  ...
  const handleUploadAttendance = () => {
    // In a real application, you'd send `filteredStudents` data for the `currentDate` and `currentHourSlot`
    // to your backend here. For this demo, we just simulate success.

    // Basic validation: Check if all visible students have a status for the current hour
    const allMarked = filteredStudents.every(
      (student) =>
        student.attendanceHistory[currentDate] &&
        student.attendanceHistory[currentDate][currentHourSlot]
    );

    if (!allMarked) {
      alert(
        "Please mark attendance for all visible students before uploading."
      );
      return;
    }

    alert(
      `Attendance for Date: ${currentDate}, Hour Slot: ${currentHourSlot} uploaded successfully!`
    );

    // Automatically move to the next hour slot
    if (currentHourSlot < totalHours) {
      setCurrentHourSlot((prevHour) => prevHour + 1);
    } else {
      alert("All hour slots (1-60) for this class have been marked!");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 font-sans antialiased mt-20">
      ...
      <main className="container mx-auto px-4">
        ...
        <section className="bg-white rounded-xl shadow-lg p-5 mb-6 border border-blue-100">
          <h2 className="text-xl font-bold mb-4 text-gray-800 text-center">
            Filter Students by Class and Course
          </h2>
          <div className="flex flex-wrap justify-center items-center gap-4">
            <div className="flex items-center gap-2">
              <label
            ...
            <div className="overflow-x-auto">
              ...
              <div className="flex justify-center mt-6">
                <button
                  ...
                  className="px-6 py-2 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg shadow-md transition duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500"
                >
                  Upload Attendance for Hour {currentHourSlot}
                </button>
              </div>
            </div>
          ) : (
              ...
              {filteredStudents.length > 0 ? (
                <>
                  <div className="flex justify-between items-center mb-4 p-3 bg-blue-50 rounded-lg shadow-sm border border-blue-100">
                    ...
                    >
                      &larr; Previous {hoursPerPage} Hours
                    </button>
